		...
		if(!player.world.isRemote && (getMaxItemUseDuration(stack) - count) % 5 == 0){
			...
			if(energy <= cageNbt.getInteger("stored_" + EnumMagicElements.ENERGY.getName()) && potential <= cageNbt.getInteger("stored_" + EnumMagicElements.POTENTIAL.getName()) && stability <= cageNbt.getInteger("stored_" + EnumMagicElements.STABILITY.getName()) && voi <= cageNbt.getInteger("stored_" + EnumMagicElements.VOID.getName())){
				if(energy + potential + stability + voi > 0){
					...
					new LooseBeamRenderable(start.x, start.y, start.z, beamVec.lengthVector(), (float) Math.toDegrees(Math.atan2(-beamVec.y, Math.sqrt(beamVec.x * beamVec.x + beamVec.z * beamVec.z))), (float) Math.toDegrees(Math.atan2(-beamVec.x, beamVec.z)), (byte) Math.sqrt(mag.getPower()), mag.getRGB().getRGB()).saveToNBT(beamNBT);
